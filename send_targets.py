...
def send_targets(
    controller, settings, targets, distances, random_x, random_y, get_left_trigger
):
    def mouse_click():
    ...
    trigger_pressed = (
        get_left_trigger(controller) > 0.5
    )  # Assuming half-press as activation threshold

    if settings["auto_aim"] == "on" and (
        trigger_pressed and settings["toggle"] == "off" or settings["toggle"] == "on"
    ):
        recoil = (
            int(settings["recoil_strength"])
            if settings["recoil"] == "on" and trigger_pressed
            else 0
        )
        ...
        if (
            ...
            and target_x <= trigger_distance
            and target_y <= trigger_distance
            and target_x >= -trigger_distance
            and target_y >= -trigger_distance
        ):
            click = 1
