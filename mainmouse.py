...
def send_targets():
    ...
    if settings['auto_aim'] == "on" and (win32api.GetKeyState(settings['activation_key']) in (-127, -128) and settings['toggle'] == "off" or (settings['toggle'] == "on")):
        recoil = int(settings['recoil_strength']) if settings['recoil'] == "on" and win32api.GetKeyState(0x01) in (-127, -128) else 0
...
root = ctk.CTk()
root.title("Lightn1ng0 AI Aimbot")
...
keybindings.title("Lightn1ng0 AI Aimbot Keybinder")
keybindings.geometry(f"400x160+{root.winfo_x() + 40}+{root.winfo_y() + 100}")
...
def main(**argv):
    ...
    pr_purple(''' 
     _        _____    _____   _    _   _______   _   _   __   _   _    _____    ___          _____    _____     ____  
    | |      |_   _|  / ____| | |  | | |__   __| | \ | | /_ | | \ | |  / ____|  / _ \        |  __ \  |  __ \   / __ \ 
    | |        | |   | |  __  | |__| |    | |    |  \| |  | | |  \| | | |  __  | | | |       | |__) | | |__) | | |  | |
    | |        | |   | | |_ | |  __  |    | |    | . ` |  | | | . ` | | | |_ | | | | |       |  ___/  |  _  /  | |  | |
    | |____   _| |_  | |__| | | |  | |    | |    | |\  |  | | | |\  | | |__| | | |_| |       | |      | | \ \  | |__| |
    |______| |_____|  \_____| |_|  |_|    |_|    |_| \_|  |_| |_| \_|  \_____|  \___/        |_|      |_|  \_\  \____/ ''')
    print("https://github.com/Lightn1ng0/Lightn1ng0_AI_Aimbot/tree/main")
    pr_yellow("\nMake sure your game is in the center of your screen!")
    pr_yellow("Follow instructions on the github page to be able to use different mouse input methods https://github.com/Lightn1ng0/Lightn1ng0_AI_Aimbot/tree/main")
    if paid_tier == 0:
        pr_red("You are a free user, most features are locked.")
    elif paid_tier == 1:
        pr_yellow("You are a byte user, some features are locked.")
    else:
        pr_green("You are either a kilobyte or gigabyte user, all features are unlocked.")
    ...
    settings['yolo_model'] = extract_original_name(os.path.splitext(argv['modelFileName'])[0])
