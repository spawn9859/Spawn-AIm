...
if not os.path.exists(f"{script_directory}/yolov5"):
    os.system(
        f"git clone https://github.com/ultralytics/yolov5 {script_directory}/yolov5"
    )
...
model, screen, overlay, canvas, random_x, random_y, arduino = (
    None,
    None,
    None,
    None,
...
def pr_red(skk):
    print(Fore.RED + skk, Style.RESET_ALL)


def pr_green(skk):
    print(Fore.GREEN + skk + Style.RESET_ALL)


def pr_yellow(skk):
    print(Fore.YELLOW + skk + Style.RESET_ALL)


def pr_blue(skk):
    print(Fore.BLUE + skk + Style.RESET_ALL)


def pr_purple(skk):
    print(Fore.MAGENTA + skk + Style.RESET_ALL)


def pr_cyan(skk):
    print(Fore.CYAN + skk + Style.RESET_ALL)


def pr_light_gray(skk):
    print(Fore.WHITE + skk + Style.RESET_ALL)


def pr_black(skk):
    print(Fore.BLACK + skk + Style.RESET_ALL)
...
def export_model():
    shutil.copy(
        f"{models_path}/{settings['yolo_model']}.pt", f"{script_directory}/temp.pt"
    )
    if settings["yolo_version"] == "v8":
        ...
        os.system(
            f"python {script_directory}/yolov5/export.py --weights {script_directory}/temp.pt --include {get_mode()} --imgsz {int(settings['height'])} {int(settings['width'])} --half --device 0"
        )
    os.remove(f"{script_directory}/temp.pt")
    if settings["yolo_mode"] == "tensorrt":
        os.remove(f"{script_directory}/temp.onnx")
        shutil.move(
            f"{script_directory}/temp.engine", f"{models_path}/{get_model_name()}"
        )
    else:
        shutil.move(
            f"{script_directory}/temp.onnx", f"{models_path}/{get_model_name()}"
        )
...
image_preview = ctk.CTkImage(
    size=(240, 240),
    dark_image=Image.open(f"{script_directory}/preview.png"),
    light_image=Image.open(f"{script_directory}/preview.png"),
)
...
░  ░  ░  ░░         ░   ▒    ░   ░     ░   ░ ░       ░   ▒  ░ ▒ ░░      ░    ░    ░ ░ ░ ░ ▒    ░      
    ...
    while True:
        ...
        with torch.no_grad():
            ...
            if (
                settings["yolo_mode"] == "pytorch"
                or settings["yolo_mode"] == "tensorrt"
            ):
